...
trait SignedValidator[F[_]] {
  ...
  def validateSignaturesWithSeedlist[A <: AnyRef](signed: Signed[A]): SignedValidationErrorOr[Signed[A]]

}

object SignedValidator {

  def make[F[_]: Async: KryoSerializer: SecurityProvider](seedlist: Option[Set[PeerId]]): SignedValidator[F] = new SignedValidator[F] {
