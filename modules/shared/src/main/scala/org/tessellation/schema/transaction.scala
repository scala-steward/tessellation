...
object transaction {
  ...
  object TransactionOrdinal {
    ...
    implicit val transactionOrder: Order[TransactionOrdinal] = (x: TransactionOrdinal, y: TransactionOrdinal) =>
      implicitly[Order[Long]].compare(x.coerce, y.coerce)

    implicit val transactionOrdering: Ordering[TransactionOrdinal] = transactionOrder.toOrdering
