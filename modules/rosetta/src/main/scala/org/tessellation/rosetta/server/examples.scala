...
class MockupData() {
  ...
  def mkGenesis(balances: Map[Address, Balance]): GlobalSnapshot =
    GlobalSnapshot(
...
address: DAG3k3VihUWMjse9LE93jRqZLEuwGd6a5Ypk4zYS
  ...
  def acceptTransactionIncrementBlock[F[_]: KryoSerializer](newTransaction: Signed[Transaction]): Either[String, Unit] =
    currentBlock.hash.left.map(_.getMessage).flatMap { cbHash =>
      ...
      val newHeight = Height(height.value + 1)
      ...
      if (value.isLeft) {
        return Left(value.swap.toOption.get.msg)
      }
      ...
      if (dstBRaw.isLeft) {
        return Left(dstBRaw.swap.toOption.get.msg)
      }
      ...
      val snapshot = GlobalSnapshot(
        ...
        Set(
          BlockAsActiveTip(
            Signed(
              DAGBlock(Set(newTransaction), NonEmptyList(BlockReference(hash.ProofsHash(""), Height(0L)), List())),
              examples.proofs
            ),
            NonNegLong(0)
          )
        ...
        Map.empty,
        Set(RewardTransaction(Address("DAG2EUdecqFwEGcgAcH1ac2wrsg8acrgGwrQefgh"), TransactionAmount(100L))),
...
object examples {
  ...
  val proofs: NonEmptyList[SignatureProof] =
