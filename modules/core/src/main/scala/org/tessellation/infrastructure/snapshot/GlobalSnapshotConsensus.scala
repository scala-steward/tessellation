...
import org.tessellation.sdk.config.types.{SnapshotConfig, StateChannelAllowanceListsConfig}
  ...
  def make[F[_]: Async: Random: KryoSerializer: SecurityProvider: Metrics: Supervisor](
    ...
    for {
      globalSnapshotStateChannelManager <- GlobalSnapshotStateChannelAcceptanceManager
        .make[F](stateChannelOrdinalDelay.some, stateChannelAllowanceLists.allowed)
