...
object GlobalSnapshotTraverseSuite extends MutableIOSuite with Checkers {
  ...
  def gst(
    ...
    val signedValidator = SignedValidator.make[IO](None)
    val blockValidator =
    ...
    val stateChannelValidator = StateChannelValidator.make[IO](signedValidator, Some(Map.empty[Address, NonEmptySet[PeerId]]))
    val validators = SdkValidators.make[IO](None, Some(Map.empty[Address, NonEmptySet[PeerId]]))
    ...
    val creator = CurrencySnapshotCreator.make[IO](
