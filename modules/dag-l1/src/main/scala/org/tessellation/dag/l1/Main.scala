...
object Main
  ...
  def run(method: Run, sdk: SDK[IO]): Resource[IO, Unit] = {
    ...
    for {
      ...
      storages <- Storages.make[IO, DAGTransaction, DAGBlock](sdkStorages, method.l0Peer).asResource
      validators = Validators.make[IO, DAGTransaction, DAGBlock](storages, seedlist)
      p2pClient = P2PClient.make[IO, DAGTransaction, DAGBlock](sdkP2PClient, sdkResources.client)
      services = Services.make[IO, DAGTransaction, DAGBlock](storages, validators, sdkServices, p2pClient, cfg)
