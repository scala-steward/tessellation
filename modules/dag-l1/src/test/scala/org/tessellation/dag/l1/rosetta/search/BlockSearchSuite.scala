...
object BlockSearchSuite extends SimpleIOSuite with Checkers {
                      ...
                      bytes =>
                        store.updateStoredBlockIndexValues(
                          ...
                          val searchIo = Database
                            ...
                            .flatMap(implicit db => IO.pure(BlockSearch.make(store)).asResource)

                          searchIo.use(
                            _.search(Some(hash), None).map(
                              _.map(x => expect.same(x.hash.value, hash.value)).getOrElse(expect(false))
                            )
                          )
                        }
                    )
  ...
  test("Test writing a Signed GlobalSnapshot - index") {
    testResource.use {
      case (store, kryo, security) =>
        implicit val kryoImplicit = kryo
        implicit val securityImplicit = security
                    ...
                    .map(
                      bytes =>
                        store.updateStoredBlockIndexValues(
                          ...
                          searchIo.use(
                            _.search(None, Some(snapshot1.height.value.value)).map(
