...
import org.tessellation.dag.l1.infrastructure.balance.storage.BalanceIndexDbStorage
...
object BalanceSearchSuite extends SimpleIOSuite with Checkers {
    ...
    forall(generatedData) { data =>
      testResource.use { store =>
        ...
        store.updateStoredValues(arguments) >> {
          val searchRes = Database
            .forAsync[IO](dbConfig)
            .flatMap(implicit db => IO.pure(BalanceSearch.make(store)).asResource)

          searchRes.use(_.search(referenceAddress, None, Some(referenceHeight)).map(expect.same(expectedSum, _)))
        }
      }
    }
  }
    ...
    forall(generatedData) { data =>
      testResource.use { store =>
        ...
        store.updateStoredValues(arguments) >> {
          ...
          searchRes.use(_.search(referenceAddress, Some(referenceHash), None).map(expect.same(referenceBalance, _)))
        }
      }
    }
  }
