...
object GlobalSnapshotStateChannelManagerSuite extends MutableIOSuite {

  type Res = (KryoSerializer[IO], SecurityProvider[IO])

  override def sharedResource: Resource[IO, GlobalSnapshotStateChannelManagerSuite.Res] =
    KryoSerializer.forAsync[IO](sharedKryoRegistrar).flatMap { ks =>
      SecurityProvider.forAsync[IO].map((ks, _))
    }

  test("valid state channel should be returned when ordinal doesn't exceed delay") { res =>
  ...
  test("valid state channel should be accepted when ordinal exceeds delay") { res =>
    ...
    for {
      stateChannelOutput <- mkStateChannelOutput(1, address, None)
      snapshotInfo = mkGlobalSnapshotInfo(SortedMap.empty)
      manager <- mkManager()
  ...
  test("valid state channel with more occurrences should be preffered") { res =>
    ...
    } yield expect.same(expected1, result) || expect.same(expected2, result)
