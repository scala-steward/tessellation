...
private[processing] trait BlockAcceptanceLogic[F[_], A <: Transaction, B <: Block[A]] {

  def acceptBlock(
    block: Signed[B],
    txChains: Map[Address, TransactionNel[A]],
...
object BlockAcceptanceLogic {
  ...
  def make[F[_]: Async: KryoSerializer: SecurityProvider, A <: Transaction, B <: Block[A]]: BlockAcceptanceLogic[F, A, B] =
    new BlockAcceptanceLogic[F, A, B] {
      ...
      def acceptBlock(
        ...
        for {
          _ <- processSignatures[F, A, B](signedBlock, context)
      ...
      def processLastTxRefs(
        txChains: Map[Address, NonEmptyList[Signed[A]]],
        context: BlockAcceptanceContext[F],
        contextUpdate: BlockAcceptanceContextUpdate
  ...
  def processSignatures[F[_]: Async: SecurityProvider, A <: Transaction, B <: Block[A]](
    signedBlock: Signed[B],
    context: BlockAcceptanceContext[F]
